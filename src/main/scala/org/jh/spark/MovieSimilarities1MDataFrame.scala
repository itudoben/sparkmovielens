...
object MovieSimilarities1MDataFrame {
  ...
  def loadMovieNames(): Map[Int, String] = {
    ...
    val lines = Source.fromFile("../data/ml-1m/movies.dat").getLines()
    for (line <- lines) {
  ...
  final case class UserMovieRating(userId: String, movieId: String, rating: Int) // timestamp
  ...
  def main(args: Array[String]) {
    ...
    val sourceFile = "../data/ml-1m/ratings.dat"
    ...
    val fields = schemaString.split(" ")
      .map(fieldName => {
        // Value is a double
        val dataType = if (fieldName == "rating") IntegerType else StringType
        StructField(fieldName, dataType, nullable = true)
      })
    val schema = StructType(fields)

    // Load the ratings into a data frame.
    val lines = sc.textFile(sourceFile).map(l => l.split("::"))
      .map(l => Row.fromSeq(Seq(l(0), l(1), l(2).toInt)))

    val ratingsDF = spark.createDataFrame(lines, schema)

    ratingsDF.show()

    // Movies are similar if they have similar movie ratings.
